...
#' `Sys.getenv("GEOCLIENT_APP_KEY")` and
#' can be used in package functions by simply typing
...
#' Key by first registering with the
#' [NYC's Developer Portal](https://api-portal.nyc.gov/signup/)
#' then selecting "Geoclient" from available APIs.
#'
...
#'   GEOCLIENT_APP_KEY that you already have in your
...
#' geoclient_api_keys(key = "9d8f7b6wh4jfgud67s89jfyw68vj38fh")
#'
...
#' geoclient_api_keys("9d8f7b6wh4jfgud67s89jfyw68vj38fh", overwrite = TRUE, install = TRUE)
#'
...
geoclient_api_keys <- function(key, overwrite = FALSE, install = FALSE){
  ...
  already_set <- any(stringr::str_detect(old_env, "GEOCLIENT_APP_KEY"))

  if (already_set && !overwrite) {
    stop_glue(
      "A GEOCLIENT_APP_KEY already exists.
  ...
  msg_glue(
    'Your API key have been stored in your .Renviron and can be accessed by \\
     Sys.getenv("GEOCLIENT_APP_KEY").
